...
-- ─────────────────────────────────────────────────────────────────────────────
-- Partial unique indexes (Section 7, BUILD_BIBLE.md)
-- Cannot be expressed in Prisma schema syntax — raw SQL only.
-- These are the DB-level concurrency backstop for the dispatch transaction.
-- They survive prisma migrate reset because they live in this migration file.
-- ─────────────────────────────────────────────────────────────────────────────

-- Rule #4: physically prevents double-dispatch of the same vehicle
CREATE UNIQUE INDEX IF NOT EXISTS one_active_trip_per_vehicle
  ON trips ("vehicleId")
  WHERE status = 'DISPATCHED';

-- Rule #4: physically prevents double-dispatch of the same driver
CREATE UNIQUE INDEX IF NOT EXISTS one_active_trip_per_driver
  ON trips ("driverId")
  WHERE status = 'DISPATCHED';

-- Rule #9: physically prevents two simultaneous OPEN maintenance logs on one vehicle
